...
{left:0,top:0};if(d.offsetParent){do a.left+=d.offsetLeft,a.top+=d.offsetTop;while(d=d.offsetParent)}return a},isActive:function(a){return a===document.activeElement&&(a.type||a.href)}};return f}(dat.utils.common);
...
GUI.prototype.createRendererSettings=function(){this.rendererFolder=this.gui.addFolder("Raytracer");var a=party.getRenderer();this.rendererFolder.add(a.settings,"exposure",-10,10).onChange(function(){party.render_dirty()});this.rendererFolder.add(a.settings,"gamma",0,10).onChange(function(){party.render_dirty()});this.rendererFolder.add(a.settings,"saturation",0,3).onChange(function(){party.render_dirty()});this.rendererFolder.add(a.settings,"contrast",0,3).onChange(function(){party.render_dirty()});this.rendererFolder.add(a.settings,
"radius",0,100).onChange(function(){party.render_dirty()});this.rendererFolder.open()};GUI.prototype.addSlider=function(a,c,e){var b=this.userFolder;"undefined"!==typeof e&&(b=e);e=c.name;var f=c.min,d=c.max;c=c.step;a=null==c||void 0==c?b.add(a,e,f,d,c):b.add(a,e,f,d);a.onChange(function(a){party.reset();party.camera.enabled=!1});a.onFinishChange(function(a){party.camera.enabled=!0});return a};
...
"update-vertex-shader":"#version 300 es\nprecision highp float;\n\nin vec3 Position;\nin vec2 TexCoord;\n\nvoid main()\n{\n    gl_Position = vec4(Position, 1.0);\n}\n"};var Renderer=function(){this.settings={};this.settings.exposure=0;this.settings.gamma=2.2;this.settings.saturation=1;this.settings.contrast=1;this.settings.radius=1;this.quadVbo=this.createQuadVbo();this.pointsVbo=null;this.pointsCount=0;this.fbo=new GLU.RenderTarget;this.shaderSources=GLU.resolveShaderSource(["render"]);this.compileShaders()};
...
this.settings.radius);this.render_program.uniformF("t",c.time);this.render_program.uniformF("lifetime",c.settings.lifetime);this.render_program.uniformF("timestep",c.settings.timestep);this.render_program.uniformF("exposure",this.settings.exposure);this.render_program.uniformF("invGamma",1/this.settings.gamma);this.render_program.uniformF("saturation",this.settings.saturation);this.render_program.uniformF("contrast",this.settings.contrast);a.enable(a.BLEND);a.blendFunc(a.ONE,a.ONE);a.enable(a.BLEND);
...
Party.prototype.gif_init=function(a,c){var e=new Blob([gifworker_code]),e=window.URL.createObjectURL(e);this.gif_frame_ms=c;this.GIF&&(delete this.GIF,this.GIF=null);this.GIF=new GIF({repeat:0,workers:20,workerScript:e,quality:30,width:Math.floor(this.width),height:Math.floor(this.height)});var b=this.getEngine();this.GIF.on("finished",function(c){console.log("gif rendering done.");var d=document.createElement("a");d.download=a;d.href=URL.createObjectURL(c);c=new MouseEvent("click");d.dispatchEvent(c);
document.body.removeChild(d);delete this.GIF;this.GIF=null;b.play()})};Party.prototype.gif_add_frame=function(){this.getGLContext();var a=GLU.canvas,c=document.createElement("canvas");c.width=a.width;c.height=a.height;c=c.getContext("2d");c.drawImage(a,0,0);this.GIF.addFrame(c,{delay:30,copy:!0})};Party.prototype.gif_render=function(){console.log("rendering gif...");this.GIF.render()};
