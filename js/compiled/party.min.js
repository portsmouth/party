...
dat.controllers.OptionController=function(a,c,e){var b=function(a,d,k){b.superclass.call(this,a,d);var f=this;this.__select=document.createElement("select");if(e.isArray(k)){var l={};e.each(k,function(d){l[d]=d});k=l}e.each(k,function(d,a){var b=document.createElement("option");b.innerHTML=a;b.setAttribute("value",d);f.__select.appendChild(b)});this.updateDisplay();c.bind(this.__select,"change",function(){f.setValue(this.options[this.selectedIndex].value)});this.domElement.appendChild(this.__select)};
...
a.uniformMatrix4fv(b,!1,e);e=party.getParticleSystem();"undefined"!==typeof e.sync_shader&&e.sync_shader(party,this.render_program);e=c.get_particle_positions();e.bind(0);this.render_program.uniformTexture("position_sampler",e);this.render_program.uniformI("NparticlesSqrt",c.settings.NparticlesSqrt);this.render_program.uniformF("radius",this.settings.radius);this.render_program.uniformF("t",c.time);this.render_program.uniformF("lifetime",c.settings.lifetime);this.render_program.uniformF("timestep",
c.settings.timestep);this.render_program.uniformF("exposure",this.settings.exposure);this.render_program.uniformF("invGamma",1/this.settings.gamma);this.render_program.uniformF("saturation",this.settings.saturation);a.enable(a.BLEND);a.blendFunc(a.ONE,a.ONE);a.enable(a.BLEND);a.drawArrays(a.POINTS,0,c.settings.Nparticles);a.disable(a.BLEND);a.finish()};Renderer.prototype.resize=function(a,c){this.width=a;this.height=c;this.radianceBuffer=new GLU.Texture(a,c,4,!0,!1,!0,null)};var Engine=function(){this.settings={};this.settings.NparticlesSqrt=100;this.settings.timestep=1/24;this.settings.lifetime=5;this.settings.max_timesteps=1E3;var a=GLU.gl;a.getParameter(a.MAX_TEXTURE_SIZE);this.quadVbo=this.createQuadVbo();this.fbo=new GLU.RenderTarget;this.shaderSources=GLU.resolveShaderSource(["initialize","update"]);this.compileShaders();this.resize(this.settings.NparticlesSqrt);this.restart();this.paused=!1};
Engine.prototype.alloc_texture_pair=function(a,c,e,b,f,d,k){return[new GLU.Texture(a,c,e,b,f,d,k),new GLU.Texture(a,c,e,b,f,d,k)]};
...
Engine.prototype.resize=function(a){a=Math.round(a);1E4<a&&(a=1E4);this.settings.NparticlesSqrt=a;this.settings.Nparticles=a*a;this.particle_positions=this.alloc_texture_pair(a,a,4,!0,!1,!0,null);this.particle_velocities=this.alloc_texture_pair(a,a,4,!0,!1,!0,null);this.particle_rngs=this.alloc_texture_pair(a,a,4,!0,!1,!0,null);this.restart()};
...
Engine.prototype.step=function(){if(this.compiled_successfully&&!this.paused){this.frame>=this.settings.max_timesteps&&this.restart();var a=party.getParticleSystem(),c=GLU.gl;c.viewport(0,0,this.settings.NparticlesSqrt,this.settings.NparticlesSqrt);this.quadVbo.bind();var e=this.frame%2,b=1-e;0==this.frame&&(this.initialize_program.bind(),this.fbo.bind(),this.fbo.drawBuffers(3),this.fbo.attachTexture(this.particle_positions[e],0),this.fbo.attachTexture(this.particle_velocities[e],1),this.fbo.attachTexture(this.particle_rngs[e],
2),this.quadVbo.draw(this.initialize_program,c.TRIANGLE_FAN),this.fbo.detachTexture(0),this.fbo.detachTexture(1),this.fbo.detachTexture(2),this.fbo.unbind());this.update_program.bind();this.update_program.uniformF("t",this.time);this.update_program.uniformF("lifetime",this.settings.lifetime);this.update_program.uniformF("dt",this.settings.timestep);this.update_program.uniformI("NparticlesSqrt",this.settings.NparticlesSqrt);"undefined"!==typeof a.sync_shader&&a.sync_shader(party,this.update_program);
this.fbo.bind();this.fbo.drawBuffers(3);this.fbo.attachTexture(this.particle_positions[b],0);this.fbo.attachTexture(this.particle_velocities[b],1);this.fbo.attachTexture(this.particle_rngs[b],2);this.particle_positions[e].bind(0);this.particle_velocities[e].bind(1);this.particle_rngs[e].bind(2);this.update_program.uniformTexture("position_sampler",this.particle_positions[e]);this.update_program.uniformTexture("velocity_sampler",this.particle_velocities[e]);this.update_program.uniformTexture("rng_sampler",
this.particle_rngs[e]);this.quadVbo.draw(this.update_program,c.TRIANGLE_FAN);this.fbo.detachTexture(0);this.fbo.detachTexture(1);this.fbo.detachTexture(2);this.fbo.unbind();c.bindTexture(c.TEXTURE_2D,null);this.frame++;this.time+=this.settings.timestep}};Engine.prototype.get_particle_positions=function(){return this.particle_positions[0]};var party,Party=function(a){party=this;this.particle_system=a;this.container=document.getElementById("container");this.render_canvas=a=document.getElementById("render-canvas");this.width=a.width;this.height=a.height;a.style.width=a.width;a.style.height=a.height;this.text_canvas=a=document.getElementById("text-canvas");this.textCtx=a.getContext("2d");window.addEventListener("resize",this,!1);this.camera=new THREE.PerspectiveCamera(45,this.width/this.height,.01,1E4);this.camera.lookAt(new THREE.Vector3(0,
...
this.textCtx.strokeText("[SPC to pause/play, ESC to restart]",14,this.textCtx.canvas.height-42);this.textCtx.fillText("[SPC to pause/play, ESC to restart]",14,this.textCtx.canvas.height-42)}};Party.prototype.getUserTextureUnitStart=function(){return 3};Party.prototype.reset=function(){this.renderer.reset()};
